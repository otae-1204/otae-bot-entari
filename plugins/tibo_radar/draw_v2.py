"""Tibo radar card renderer v2 — light "signal log" style dashboard cards.

A drop-in visual replacement for ``draw.py``.  The public API is identical
(``render_card``, ``CardSection``, ``post_sections``, ``event_sections`` and the
``AMBER`` / ``CYAN`` / ``GREEN`` constants), so the plugin can switch renderer by
changing a single import line — no other code needs to change.

Visual language of v2:
- light paper background with a subtle dot grid
- deep-navy header band with a radar scope emblem and gradient rule
- white floating panels with soft shadows and a colored accent bar
- dark-on-light status chips, glowing status LED, stat tiles and a 24h chart

Row classification (timeline / feed / kv / quote / link rows) is kept in sync
with the section content produced by ``plugins/tibo_radar/__init__.py``.
"""
...
# Legacy constant names kept for compatibility with existing imports.
# v2 uses a light palette, so these accents are darker, print-friendly tones.
BG = (243, 245, 248)
PANEL = (255, 255, 255)
...
NAVY = (13, 24, 44)
NAVY_SOFT = (24, 38, 64)
HEADER_TEXT = (238, 243, 250)
HEADER_MUTED = (158, 174, 198)
SUBTLE = (66, 82, 106)
FAINT = (146, 158, 178)
BLUE = (46, 106, 220)
SHADOW = (190, 200, 216)
...
def _chip(draw: ImageDraw.ImageDraw, x: float, y: float, text: str, color, font=None, *, alpha: int = 22) -> float:
    font = font or FONT_TAG
    width = font.getlength(text) + 22
    height = getattr(font, "size", 16) + 12
    draw.rounded_rectangle((x, y, x + width, y + height), radius=height / 2, fill=(*color, alpha), outline=(*color, 130), width=1)
    _dtext(draw, (x + 11, y + 5), text, font, color)
    return width


def _led(draw: ImageDraw.ImageDraw, cx: float, cy: float, color, r: float = 7) -> None:
    for radius, alpha in ((r + 10, 26), (r + 5, 56), (r, 240)):
...
def _draw_background(draw: ImageDraw.ImageDraw, height: int) -> None:
    draw.rectangle((0, 0, CANVAS_W, height), fill=BG)
    # Subtle dot grid for the paper texture.
    step = 30
    for gy in range(HEADER_H + 10, height, step):
        for gx in range(18, CANVAS_W, step):
            draw.point((gx, gy), fill=(203, 213, 226))
            draw.point((gx + 1, gy), fill=(213, 221, 232))


def _draw_scope_emblem(draw: ImageDraw.ImageDraw, cx: float, cy: float) -> None:
    """Radar scope emblem on the navy header band."""
    ring = (94, 200, 224)
    for radius, alpha in ((30, 90), (52, 60), (74, 42)):
        draw.ellipse((cx - radius, cy - radius, cx + radius, cy + radius), outline=(*ring, alpha), width=2)
    # Sweep wedge + needle.
    draw.pieslice((cx - 52, cy - 52, cx + 52, cy + 52), start=208, end=252, fill=(94, 200, 224, 34))
    angle = math.radians(252)
    draw.line((cx, cy, cx + 74 * math.cos(angle), cy + 74 * math.sin(angle)), fill=(*ring, 160), width=2)
    draw.line((cx - 74, cy, cx + 74, cy), fill=(*ring, 26), width=1)
    draw.line((cx, cy - 74, cx, cy + 74), fill=(*ring, 26), width=1)
    # Blips.
    for radius, degrees, color in ((52, 160, GREEN), (74, 296, AMBER), (30, 340, (94, 200, 224))):
        a = math.radians(degrees)
        bx, by = cx + radius * math.cos(a), cy + radius * math.sin(a)
        draw.ellipse((bx - 7, by - 7, bx + 7, by + 7), fill=(*color, 46))
        draw.ellipse((bx - 3, by - 3, bx + 3, by + 3), fill=(*color, 235))
...
def _draw_header(draw: ImageDraw.ImageDraw, title: str, subtitle: str, page: str) -> None:
    band_h = HEADER_H - 26
    # Navy band with a soft vertical blend.
    for y in range(band_h):
        ratio = y / max(1, band_h - 1)
        color = (
            int(NAVY[0] + (NAVY_SOFT[0] - NAVY[0]) * ratio),
            int(NAVY[1] + (NAVY_SOFT[1] - NAVY[1]) * ratio),
            int(NAVY[2] + (NAVY_SOFT[2] - NAVY[2]) * ratio),
        )
        draw.line((0, y, CANVAS_W, y), fill=color)
    _draw_scope_emblem(draw, CANVAS_W - 118, band_h // 2 + 6)

    header_cyan = (94, 200, 224)
    kicker = "CODEX RADAR"
    kicker_w = sum(FONT_KICKER.getlength(ch) + 3 for ch in kicker) - 3 + 28
    draw.rounded_rectangle((PAD_X, 40, PAD_X + kicker_w, 72), radius=16, fill=(*header_cyan, 26), outline=(*header_cyan, 130), width=1)
    _draw_tracked(draw, PAD_X + 14, 46, kicker, FONT_KICKER, header_cyan)
    if page:
        label = f"PAGE {page}"
        width = FONT_TAG.getlength(label) + 24
        _chip(draw, CANVAS_W - PAD_X - width - 190, 44, label, HEADER_MUTED, FONT_TAG, alpha=20)
    _dtext(draw, (PAD_X, 84), _clip_width(title, FONT_TITLE, 700), FONT_TITLE, HEADER_TEXT)
    _dtext(draw, (PAD_X, 148), _clip_width(_clip(subtitle, 110), FONT_SMALL, CANVAS_W - PAD_X * 2 - 180), FONT_SMALL, HEADER_MUTED)
    # Gradient rule under the band.
    rule_y = band_h - 2
    segment = 460
    for i in range(segment):
        alpha = int(235 * (1 - i / segment) ** 1.5)
        draw.line((PAD_X + i, rule_y, PAD_X + i + 1, rule_y), fill=(*CYAN, alpha), width=3)
    draw.line((PAD_X + segment, rule_y + 1, CANVAS_W - PAD_X, rule_y + 1), fill=(120, 140, 165, 60), width=1)


def _draw_footer(draw: ImageDraw.ImageDraw, y: float) -> None:
    draw.line((PAD_X, y, CANVAS_W - PAD_X, y), fill=(148, 163, 184, 90), width=1)
...
def _draw_tiles(draw: ImageDraw.ImageDraw, x: float, y: float, w: float, tiles: list[tuple[str, str, str]]) -> None:
    ...
    for index, (caption, value, unit) in enumerate(tiles):
        ...
        draw.rounded_rectangle((tx, y, tx + tile_w, y + 96), radius=14, fill=PANEL_ALT, outline=(*LINE, 220), width=1)
...
def _draw_bars(draw: ImageDraw.ImageDraw, x: float, y: float, w: float, data: dict[int, int]) -> None:
    ...
    for hour in range(slots):
        ...
        draw.rounded_rectangle((bx, base - bar_h, bx + bar_w, base), radius=3, fill=(*color, 235 if is_max else 150))
        if is_max:
            _dtext(draw, (bx + bar_w / 2, base - bar_h - 22), str(value), FONT_TINY, GREEN, anchor="ma")
    draw.line((x, base + 1, x + w, base + 1), fill=(100, 116, 139, 90), width=1)
...
def _draw_row(draw: ImageDraw.ImageDraw, row: dict, x: float, y: float, w: float) -> None:
    ...
    if kind == "hero":
        ...
        _chip(draw, x + row["indent"], y, row["key"], _KEY_COLORS.get(row["key"], MUTED), FONT_TAG, alpha=20)
        yy = y + 3
        for line in row["lines"]:
        ...
        draw.line((x + row["indent"], y + 30, x + row["indent"] + FONT_SMALL.getlength(url), y + 30), fill=(*BLUE, 110), width=1)
    elif kind == "text":
        font = FONT_QUOTE if row["subtle"] else FONT_BODY
...
def _draw_panel(draw: ImageDraw.ImageDraw, x: float, y: float, w: float, panel: dict) -> None:
    height = panel["height"]
    accent = panel["accent"]
    # Soft drop shadow, then the white card body.
    draw.rounded_rectangle((x + 3, y + 5, x + w + 3, y + height + 5), radius=18, fill=(*SHADOW, 70))
    draw.rounded_rectangle((x, y, x + w, y + height), radius=18, fill=PANEL, outline=(*LINE, 230), width=1)
    if panel["hero"]:
        draw.rounded_rectangle((x, y, x + w, y + height), radius=18, fill=(*panel["hero"], 12), outline=(*panel["hero"], 90), width=1)
    # Accent bar along the left edge.
    draw.rounded_rectangle((x, y + 14, x + 6, y + height - 14), radius=3, fill=(*accent, 235))
    inner_x = x + PANEL_PAD_X
    inner_w = w - PANEL_PAD_X * 2
    draw.rounded_rectangle((inner_x, y + 26, inner_x + 5, y + 52), radius=2, fill=(*accent, 235))
    _dtext(draw, (inner_x + 17, y + 24), _clip_width(panel["title"], FONT_SECTION, inner_w - 30), FONT_SECTION, TEXT)
    draw.line((inner_x, y + 68, inner_x + inner_w, y + 68), fill=(100, 116, 139, 60), width=1)
    ...
    for row in panel["rows"]:
        if conn_x is not None:
            draw.line((conn_x, conn_y, conn_x, yy), fill=(*accent, 70), width=2)
