"""Tibo radar card renderer — X dark-mode design system for all reply images.

This is the plugin's unified renderer: a single visual identity ("X signal
session") applied to every reply card — 总览 / 状态 / 最近 / 历史 — and it
re-exports the feed renderer for 动态 (``render_xfeed`` from ``draw_xfeed``).

Visual language:
- pure-black header band with the X glyph emblem and a blue gradient rule
- near-black page background, rounded panels with a left accent rail
- the rail carries a glowing state LED: the "signal lane" signature element
- semantic signal colors: green confirmed/direct, amber window/suspected/
  indirect, red rejected, gray no-signal, X blue for links and evidence

The public API is identical to ``draw.py`` (``render_card``, ``CardSection``,
``post_sections``, ``event_sections``, color constants), so switching the
plugin to this renderer is a one-line import change.
"""
...
# Imported for re-export so callers can use a single module.
from .draw_xfeed import render_xfeed  # noqa: F401  (re-export)
...
_PHASE_STYLES = {"睡觉": X_BLUE, "上班": CYAN, "可能在线": GREEN}
_QUOTE_KEYS = {"原文", "翻译", "解读", "摘要", "证据", "证据原文"}
_BOLD_KEYS = {"距今"}
_KEY_COLORS = {"原文": X_BLUE, "翻译": CYAN, "解读": AMBER}
...
def _chip(draw: ImageDraw.ImageDraw, x: float, y: float, text: str, color, font=None, *, alpha: int = 30, edge: int = 150) -> float:
...
def _led(draw: ImageDraw.ImageDraw, cx: float, cy: float, color, r: float = 7) -> None:
    for radius, alpha in ((r + 10, 22), (r + 5, 50), (r, 240)):
...
def _draw_header(draw: ImageDraw.ImageDraw, title: str, subtitle: str, page: str) -> None:
    band_h = HEADER_H - 26
    # Pure-black band with a soft vertical blend.
    ...
    kicker = "X · PUBLIC SIGNAL"
    kicker_w = sum(FONT_KICKER.getlength(ch) + 3 for ch in kicker) - 3 + 28
    draw.rounded_rectangle((PAD_X, 40, PAD_X + kicker_w, 72), radius=16, fill=(*X_BLUE, 24), outline=(*X_BLUE, 140), width=1)
    pos = PAD_X + 14
    for ch in kicker:
        _dtext(draw, (pos, 46), ch, FONT_KICKER, X_BLUE)
        pos += FONT_KICKER.getlength(ch) + 3

    if page:
        label = f"PAGE {page}"
        width = FONT_TAG.getlength(label) + 24
        _chip(draw, CANVAS_W - PAD_X - width - 190, 44, label, MUTED, FONT_TAG, alpha=18, edge=90)

    _dtext(draw, (PAD_X, 86), _clip_width(title, FONT_TITLE, 700), FONT_TITLE, TEXT)
    _dtext(draw, (PAD_X, 150), _clip_width(_clip(subtitle, 110), FONT_SMALL, CANVAS_W - PAD_X * 2 - 180), FONT_SMALL, MUTED)
...
def _draw_tiles(draw: ImageDraw.ImageDraw, x: float, y: float, w: float, tiles: list[tuple[str, str, str]]) -> None:
    ...
    for index, (caption, value, unit) in enumerate(tiles):
        ...
        draw.rounded_rectangle((tx, y, tx + tile_w, y + 96), radius=14, fill=(*PANEL_ALT, 240), outline=(*PANEL_EDGE, 170), width=1)
...
def _draw_bars(draw: ImageDraw.ImageDraw, x: float, y: float, w: float, data: dict[int, int]) -> None:
    ...
    for hour in range(slots):
        ...
        color = GREEN if is_max else X_BLUE
...
def _draw_row(draw: ImageDraw.ImageDraw, row: dict, x: float, y: float, w: float) -> None:
    ...
    if kind == "hero":
        ...
        url = row["url"]
        _dtext(draw, (x + row["indent"], y + 2), url, FONT_SMALL, X_BLUE)
        draw.line((x + row["indent"], y + 30, x + row["indent"] + FONT_SMALL.getlength(url), y + 30), fill=(*X_BLUE, 110), width=1)
    elif kind == "text":
        font = FONT_QUOTE if row["subtle"] else FONT_BODY
...
def _draw_panel(draw: ImageDraw.ImageDraw, x: float, y: float, w: float, panel: dict) -> None:
    height = panel["height"]
    draw.rounded_rectangle((x, y, x + w, y + height), radius=18, fill=(*PANEL, 246), outline=(*PANEL_EDGE, 170), width=1)
    if panel["hero"]:
        draw.rounded_rectangle((x, y, x + w, y + height), radius=18, fill=(*panel["hero"], 13), outline=(*panel["hero"], 90), width=1)
    # Signal rail: the left accent bar carried by every panel.
    draw.rounded_rectangle((x, y + 14, x + 6, y + height - 14), radius=3, fill=(*panel["accent"], 235))
    ...
    draw.line((inner_x, y + 68, inner_x + inner_w, y + 68), fill=(*LINE, 18), width=1)
