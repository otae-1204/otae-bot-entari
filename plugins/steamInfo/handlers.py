from plugins.steamInfo.draw import (
    check_font,
    fetch_avatar,
    image_to_bytes,
    draw_player_status,
...
from plugins.steamInfo.data_source import (
    BindData,
    SteamInfoData,
    ParentData,
    DisableParentData,
...
from plugins.steamInfo.steam import (
    get_steam_id,
...
from plugins.steamInfo.models import PlayerSummaries, Player
from plugins.steamInfo.config import Config
...
from otae_bot.adapters.entari import prompt, get_bot, on_ready
import aiohttp
import asyncio
import re
import time
...
from otae_bot.adapters.entari import inject
from arclet.entari import Event, Account as Bot, Image
from otae_bot.config.settings import SYSTEM_PROXY, Config as GlobalConfig, _env
...
async def _resolve_steam_game_query(query: str) -> tuple[str, Optional[str], Optional[dict]]:
    ...
    if not candidates:
        ...
        force_confirm = bool(candidates)
        if len(candidates) > 1:
            from plugins.steamInfo.steam import write_app_ambiguous_cache
