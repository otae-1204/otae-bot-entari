...
async def draw_operator_card(view: OperatorView) -> bytes:
    html_path = _write_temp_html(await render_operator_card_html(view))
    try:
...
async def draw_weapon_card(view: WeaponView) -> bytes:
    html_path = _write_temp_html(await render_weapon_card_html(view))
    try:
        return await screenshot_web_element(
            html_path.resolve().as_uri(),
            ".weapon-card",
...
async def render_operator_card_html(view: OperatorView) -> str:
    portrait = await _image_data_url(view.portrait_url or view.round_icon_url or view.icon_url)
    skill_icons = {skill.icon_id: await _image_data_url(skill_icon_url(skill.icon_id)) for skill in view.skills if skill.icon_id}
    talent_icons = {effect.effect_id: await _image_data_url(effect.icon_url) for effect in view.talents if effect.icon_url}
    term_styles = merged_term_styles(view)
    term_icons = {term: await _image_data_url(style.icon_url) for term, style in term_styles.items() if style.icon_url}
    return _render_html(view, portrait, skill_icons, talent_icons, term_styles, term_icons)


async def render_weapon_card_html(view: WeaponView) -> str:
    weapon_img = await _image_data_url(view.icon_url)
    rich_icons: dict[str, str] = {}
    for icon_url in _weapon_rich_icon_urls_used(view):
        if icon_url and icon_url not in rich_icons:
            rich_icons[icon_url] = await _image_data_url(icon_url)
    return _render_weapon_html(view, weapon_img, rich_icons)
...
.info-box {{ min-height: 50px; border-left: 5px solid #171b1f; background: rgba(255,255,255,.58); padding: 6px 8px; }}
.info-label {{ font-size: 12px; color: #697279; font-weight: 800; }}
.info-value {{ margin-top: 3px; font-size: 18px; font-weight: 900; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }}
...
.potential-list {{ display: grid; gap: 7px; }}
.potential-item {{
  display: grid;
...
.potential-title {{ font-size: 16px; line-height: 1.05; font-weight: 900; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }}
.potential-desc {{ margin-top: 3px; color: #364047; font-size: 12px; line-height: 1.28; font-weight: 700; overflow: visible; white-space: normal; word-break: break-word; overflow-wrap: anywhere; }}
...
.portrait img {{ width: 116%; height: 116%; object-fit: contain; object-position: center center; filter: drop-shadow(0 28px 30px rgba(29,34,39,.30)); }}
...
.skill-grid {{ display: grid; grid-template-columns: 1fr 1fr; gap: 8px 10px; }}
.skill-card {{ min-height: 244px; border: 1px solid rgba(23,27,31,.24); background: linear-gradient(180deg, rgba(255,255,255,.78), rgba(234,237,238,.84)); padding: 7px; position: relative; overflow: visible; }}
...
.skill-name {{ font-size: 19px; line-height: 1.02; font-weight: 900; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; }}
.skill-cat {{ font-size: 12px; color: #6b7379; font-weight: 900; margin-top: 2px; }}
.skill-desc {{ margin: 7px 0 0 13px; min-height: 38px; color: #303941; font-size: 12px; line-height: 1.2; font-weight: 700; overflow: visible; }}
.skill-meta {{ margin: 6px 0 0 13px; display: grid; grid-template-columns: repeat(2, 1fr); gap: 4px; }}
.skill-meta:empty {{ display: none; }}
.skill-meta span {{ min-height: 22px; padding: 3px 5px; background: rgba(23,27,31,.08); color: #313940; font-size: 11.5px; font-weight: 900; text-align: center; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }}
.skill-meta strong {{ color: #11161a; background: #ffd000; padding: 0 3px; }}
.metric-table {{ margin: 6px 0 0 13px; display: grid; grid-template-columns: 100px repeat(4, minmax(0, 1fr)); grid-auto-rows: minmax(22px, auto); border-top: 1px solid rgba(23,27,31,.2); border-left: 1px solid rgba(23,27,31,.2); }}
.metric-table div {{ min-height: 22px; padding: 3px 4px; border-right: 1px solid rgba(23,27,31,.2); border-bottom: 1px solid rgba(23,27,31,.2); font-size: 12px; font-weight: 800; line-height: 1.05; overflow: visible; }}
...
.effect-grid {{ display: grid; grid-template-columns: 1fr 1fr; gap: 7px 10px; }}
.effect-card {{ min-height: 72px; display: grid; grid-template-columns: 40px 1fr; gap: 8px; border: 1px solid rgba(23,27,31,.24); background: rgba(255,255,255,.62); padding: 7px; }}
.effect-title {{ font-size: 16px; line-height: 1.03; font-weight: 900; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }}
.effect-desc {{ margin-top: 3px; font-size: 12px; line-height: 1.18; color: #333b41; font-weight: 700; overflow: visible; }}
.footer-line {{ margin-top: auto; padding-top: 12px; display: flex; justify-content: flex-end; color: #697279; font-size: 13px; line-height: 1.1; font-weight: 900; }}
</style>
</head>
<body>
<div class="endfield-card">
  <section class="rail">
    ...
    <div class="info-grid">
      ...
      {info_box("种族", view.species or "未知")}
    </div>
    {tag_block(view.tags)}
  ...
  <section class="panel">
    <div class="panel-title">
      <h1>干员数据详表</h1>
      <div class="columns"><span>技能等级</span>{''.join(f'<span>{label}</span>' for _, label in LEVEL_COLUMNS)}</div>
    </div>
...
def _render_weapon_html(view: WeaponView, weapon_img: str, rich_icons: dict[str, str]) -> str:
    card_min_height = 720
...
.weapon-card {{ position:relative; width:{CARD_WIDTH}px; min-height:{card_min_height}px; height:{card_min_height}px; overflow:hidden; background:linear-gradient(90deg,rgba(29,34,39,.075) 1px,transparent 1px) 0 0/40px 40px,linear-gradient(0deg,rgba(29,34,39,.075) 1px,transparent 1px) 0 0/40px 40px,linear-gradient(135deg,#f7f8f4 0%,#e8ebed 58%,#cfd5d9 100%); }}
.rail {{ position:absolute; z-index:3; left:28px; top:28px; bottom:28px; width:440px; padding:20px 20px; border:1px solid rgba(29,34,39,.30); background:rgba(247,248,246,.93); }}
...
.meta-grid {{ display:grid; grid-template-columns:132px 86px 104px; gap:8px; margin-top:18px; width:max-content; }} .info-box {{ min-height:56px; border-left:5px solid #171b1f; background:rgba(255,255,255,.58); padding:7px 9px; }} .info-label {{ font-size:12px; color:#697279; font-weight:950; }} .info-value {{ margin-top:3px; font-size:19px; line-height:1.1; font-weight:950; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }}
.visual {{ position:absolute; z-index:4; left:54px; top:318px; bottom:55px; width:390px; height:auto; pointer-events:none; }} .visual-frame {{ position:absolute; left:0; top:0; width:374px; height:100%; border:0; background:linear-gradient(180deg,rgba(255,255,255,.16),rgba(0,0,0,.015)); clip-path:polygon(0 0,93% 0,100% 7%,100% 100%,7% 100%,0 93%); }} .weapon-orbit {{ position:absolute; left:23px; top:9%; width:331px; height:76%; border:0; transform:skewY(-5deg); background:radial-gradient(circle at 52% 48%,rgba(255,255,255,.78),rgba(255,255,255,0) 58%); }}
.weapon-img {{ position:absolute; left:2px; top:0; width:382px; height:100%; object-fit:contain; object-position:center center; filter:drop-shadow(0 32px 20px rgba(23,27,31,.30)); transform:rotate(-7deg); }}
...
.panel {{ position:absolute; z-index:3; right:24px; top:28px; width:1070px; height:auto; display:flex; flex-direction:column; background:rgba(247,248,246,.95); border:1px solid rgba(23,27,31,.32); padding:14px 18px 14px; box-shadow:-12px 18px 44px rgba(23,27,31,.14); }} .panel-title {{ display:block; border-bottom:4px solid #171b1f; padding-bottom:10px; margin-bottom:10px; }} .panel-title h1 {{ margin:0; font-size:32px; line-height:1; font-weight:950; }}
.skill-stack {{ min-height:0; display:grid; grid-template-columns:0.76fr 1.24fr; grid-template-rows:auto auto; gap:10px; align-items:start; }} .skill-card {{ --row-font:13px; --label-font:12px; --row-line:1.18; position:relative; display:flex; flex-direction:column; padding:12px 16px 10px 18px; border:1px solid rgba(23,27,31,.18); background:#f4f5f7; overflow:hidden; }} .skill-card.s1 {{ grid-column:1; grid-row:1; }} .skill-card.s2 {{ grid-column:1; grid-row:2; }} .skill-card.s3 {{ grid-column:2; grid-row:1 / 3; }} .skill-head {{ display:block; flex:0 0 auto; padding-bottom:5px; }} .skill-name {{ font-size:23px; line-height:1.05; font-weight:950; }} .skill-card.s1 .skill-name,.skill-card.s2 .skill-name {{ font-size:24px; }} strong {{ color:#f2b900; background:transparent; padding:0 1px; }} .term {{ color:#bd7f42; font-weight:950; border-bottom:1px dotted rgba(189,127,66,.55); }} .term-icon {{ display:inline-block; object-fit:contain; margin:0 1px 0 2px; vertical-align:-2px; filter:none; }} .rich-style,.vup,.info-note {{ display:inline; }} .info-note {{ color:#59636a; font-weight:900; }}
.frontend-level-list {{ clear:both; display:grid; grid-template-rows:repeat(9,minmax(0,1fr)); flex:1 1 auto; min-height:0; border-top:1px solid rgba(23,27,31,.10); }} .level-row {{ display:grid; grid-template-columns:45px 1fr; gap:10px; align-items:center; border-bottom:1px solid rgba(23,27,31,.08); }} .level-row:last-child {{ border-bottom:0; }} .level-label {{ padding:0; color:#536071; font-size:var(--label-font); line-height:1; font-weight:900; align-self:center; }} .level-desc {{ padding:0; align-self:center; color:#17202a; font-size:var(--row-font); line-height:var(--row-line); font-weight:850; }} .frontend-level-list.short .level-row {{ min-height:26px; }} .skill-card.s1 .level-label,.skill-card.s2 .level-label {{ font-size:var(--label-font); }} .skill-card.s1 .level-desc,.skill-card.s2 .level-desc {{ font-size:var(--row-font); line-height:var(--row-line); }} .frontend-level-list.long .level-row {{ grid-template-columns:42px 1fr; min-height:50px; }} .frontend-level-list.long .level-label {{ font-size:var(--label-font); }} .frontend-level-list.long .level-desc {{ font-size:var(--row-font); line-height:var(--row-line); letter-spacing:-.022em; }}
...
<div class="weapon-card">
  ...
  <section class="panel">
    ...
    <div class="footer-line"><span>数据来源 api.fz.wiki · {esc(view.title)}</span><span>更新 {esc(view.source_version or "--")}</span></div>
  </section>
</div>
<script>
(function() {{
  ...
  const natural = cards.map(c => Math.ceil(c.scrollHeight));
  let h1 = natural[0], h2 = natural[1], h3 = natural[2];
  const leftTotal = h1 + h2 + gap;
  if (h3 > leftTotal) {{
    const add = (h3 - leftTotal) / 2;
    h1 += add; h2 += add;
  }} else {{
    h3 = leftTotal;
  }}
  function applySkillHeight(cardEl, height, baseFont, baseLabel, baseRow) {{
    const finalHeight = Math.ceil(height);
    cardEl.style.height = finalHeight + 'px';
    const list = cardEl.querySelector('.frontend-level-list');
    const head = cardEl.querySelector('.skill-head');
    const cs = getComputedStyle(cardEl);
    const paddings = parseFloat(cs.paddingTop) + parseFloat(cs.paddingBottom) + 2;
    const listHeight = Math.max(0, finalHeight - Math.ceil(head.offsetHeight) - paddings);
    list.style.height = listHeight + 'px';
    const rowH = Math.max(baseRow, listHeight / 9);
    const grow = Math.max(1, rowH / baseRow);
    const font = Math.min(baseFont * 1.22, baseFont * Math.pow(grow, 0.38));
    const label = Math.min(baseLabel * 1.18, baseLabel * Math.pow(grow, 0.30));
    cardEl.style.setProperty('--row-font', font.toFixed(2) + 'px');
    cardEl.style.setProperty('--label-font', label.toFixed(2) + 'px');
    cardEl.style.setProperty('--row-line', rowH < baseRow * 1.08 ? '1.16' : '1.20');
  }}
  applySkillHeight(s1, h1, 14, 12.5, 26);
  applySkillHeight(s2, h2, 14, 12.5, 26);
  applySkillHeight(s3, h3, 10.8, 11.2, 50);
  const stackHeight = Math.ceil(Math.max(h1 + h2 + gap, h3));
  stack.style.height = stackHeight + 'px';
  const panelHeight = Math.ceil(panel.scrollHeight);
  panel.style.height = panelHeight + 'px';
  const cardHeight = Math.max({card_min_height}, panelHeight + 56);
...
def estimate_card_height(view: OperatorView) -> int:
    ...
    left_height = 28 + 20 + 31 + 58 + 24 + 118 + 10 + 37 + len(view.potentials[:5]) * 78 + 78 + 28
    return min(CARD_MAX_HEIGHT, max(CARD_MIN_HEIGHT, int(max(panel_height + 56, left_height))))
...
def metric_table(skill: SkillView) -> str:
    ...
    if not cells:
        cells = ['<div class="metric-name">效果</div>'] + ['<div class="metric-value">--</div>' for _ in LEVEL_COLUMNS]
    return '<div class="metric-table">' + "".join(cells) + "</div>"
...
def skill_metric_rows(skill: SkillView) -> list[tuple[str, list[str]]]:
    if skill.category == "普攻":
        rows = {
            "普攻倍率": ["--"] * len(LEVEL_COLUMNS),
            "处决攻击倍率": ["--"] * len(LEVEL_COLUMNS),
            "下落攻击倍率": ["--"] * len(LEVEL_COLUMNS),
        }
...
def highlight_terms(
    ...
    while index < len(text):
        ...
        if term:
            ...
            color_attr = f' style="--term-color: {esc_attr(style.color)}"' if style.color else ""
            rendered.append(
