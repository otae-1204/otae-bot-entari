...
玩家只能获得以下每组任务评价档案的一个版本，因此 `_1` / `_2` / `_3` 每组按一份
收集档案计数（不按名称全局去重）：

- `nar_sm1l1m4_hatman_*`：哈特曼记录·其一
- `nar_sm1l1m5_hatman_*`：哈特曼记录·其二
- `nar_sm1l1m5_Alexander_*`：亚历山大记录·其一
- `nar_sm1l1m5_Hans_*`：老汉斯记录·其一

仅匹配上述 12 个明确 ID。展示优先选 `_1`，缺失时选已有版本；计数与版本对比统一
使用 `_1` 作为收集 ID。新快照、旧磁盘快照读取、个人进度和版本新增均使用相同口径，
旧快照无需手动删除或重新下载。基线中的其他版本 ID 同样归一化，避免换版误报新增。

AKEData `1.5.3@10506507-7` 实测：原始 509 → 可收集 **501**，电子档案 96 → **88**；
见闻辑录 **399**、音像存档 **26**、中枢档案 **76**。规范化后的 501 个 ID 与本次抓取
的 Open Endfield Map 已部署档案目录一致。

以下 2026-09-10 的 495 等数字是旧构建的原始条目数，不是修正后的可收集上限。
...
进度卡用 `missing = 去除互斥版本后的快照总数 − docNum`，`docNum > 总数` 时显示口径异常提示而非负数。

**旧记录更正**：2026-09-10 的 docNum=472 与截图中已收集数量 372 + 26 + 74 一致，
但截图未证明全库上限为 495；此前据原始表计数推断 missing=23 不成立。全库分母现按
上面的互斥版本规则修正，森空岛返回的玩家已收集数量保持原值。
...
- 物品卡背景参考游戏截图：上深下浅的垂直灰色渐变、淡测量刻度/大框线与独立名称栏。
  刻度底纹在 Icon 后方，不叠加贯穿物品的扫描竖线；左上黑色装饰标签已移除。
...
- 本次 `CARD_RENDER_VERSION` 已更新为 `endfield-card-v50`；以后改版式继续递增。
